    ...
    def PopulatesInputsWithList(self):
        ...
        for _value in self.value:
            if isinstance(_value, dict):
                if CheckIfVector(_value):
                    _obj = JsonStrut(str(index), _value, self)
                    self.scrollAreaLayout.addLayout(_obj.GetUILayout())
    ...
    def MakeUI(self):
        for key in self.value:
            if isinstance(self.value[key], dict):
                if CheckIfVector(self.value[key]):
            ...
            self.baseLayout.addLayout(self.childObjects[key].GetUILayout())
...
class JsonToUI(jsonBase):
    def __init__(self, _jsonLocation):
        self.jsonLocation = _jsonLocation
        _value = JsonManager.GetJsonToDict(self.jsonLocation)
    ...
    def UpdateCurrentUIWithNewJsonValuse(self, _folderPath):
        ...
        if os.path.exists(_newJsonPath):
            ...
            _newData = JsonManager.GetJsonToDict(_newJsonPath)
    ...
    def SaveJson(self, _path):
        ...
        JsonManager.SaveJsonData(_location, _name, self.value)


def CheckIfVector(_strut):
